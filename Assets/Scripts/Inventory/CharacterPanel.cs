...
public class CharacterPanel : Inventory {
    ...
    public void PutOn(Item item)
    {
        ...
        {
            EquipmentSlot equitmentSlot = (EquipmentSlot)slot;
            if (equitmentSlot.IsRightItem(item))
            {
                if (equitmentSlot.transform.childCount > 0)
                {
                    ItemUI currentItemUI = equitmentSlot.transform.GetChild(0).GetComponent<ItemUI>();
